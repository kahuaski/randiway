...
export default function Header() {
  ...
  return (
    <header className="w-full bg-white font-sans tracking-wide relative z-50">
        ...
        <div className="max-w-350 mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 md:h-24 gap-4">
            <div className="flex items-center gap-3 md:gap-0">
              ...
              <Link
                href="/"
                className="inline-flex items-center text-zinc-900 text-2xl md:text-4xl font-extrabold tracking-tighter hover:opacity-80 transition-opacity"
              >
            ...
            <div className="flex items-center gap-4 md:gap-8">
              ...
              <div className="flex items-center gap-4 md:gap-5 text-zinc-800">
                <Link
                  href="/favoritos"
                  className="hidden md:block hover:text-emerald-700 transition-colors"
                >
      ...
      />
      <div
        className={`fixed top-0 left-0 h-full w-[85%] max-w-[320px] bg-white z-50 transform transition-transform duration-300 ease-in-out md:hidden flex flex-col ${isMenuOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}
      >
        <div className="flex items-center justify-between p-4 border-b border-stone-100 bg-stone-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <Link
                href="/login"
                className="text-zinc-900 font-bold text-sm hover:text-emerald-700"
                onClick={() => setIsMenuOpen(false)}
              >
                Ingresa a tu cuenta
              </Link>
          ...
          <button
            onClick={() => setIsMenuOpen(false)}
            className="p-2 text-stone-400 hover:text-zinc-900 bg-white rounded-full shadow-sm"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          <Link
            href="/ofertas"
            className="flex items-center justify-between px-5 py-4 text-emerald-700 font-bold text-sm tracking-wide border-b border-stone-50"
            onClick={() => setIsMenuOpen(false)}
          >
        ...
        <div className="p-4 border-t border-stone-100 bg-stone-50 space-y-3">
          <Link
            href="/favoritos"
            className="flex items-center gap-3 text-zinc-700 font-medium text-sm p-2 hover:text-emerald-700"
            onClick={() => setIsMenuOpen(false)}
          >
